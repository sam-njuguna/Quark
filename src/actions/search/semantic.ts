...
import { generateEmbedding, embeddingToString, findSimilarWork } from "@/lib/embeddings";
import { z } from "zod";
...
export async function semanticSearch(rawInput: unknown) {
  ...
  const results = await db
    ...
    .from(work)
    .where(input.teamId ? eq(work.teamId, input.teamId) : undefined)
...
export async function getSimilarWork(workId: string, limit = 5) {
  await requireUser();
  return findSimilarWork(workId, "", limit);
}
